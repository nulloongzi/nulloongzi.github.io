...
- **연출**: **`rally3d.html` = v4 "누룽지의 눈으로"** — `docs/website_design.md`(디자인 규칙서, 필독)와 사용자 검수 시안(`docs/design/look-*.png`) 그대로 구현한 켄마 시점 몽타주. 11개 비트(hero→서브 홀드→토스업→실루엣 아치→서브 타격→볼 플라이트→리시브→토스→스파이크 어프로치→타격→밥그릇 착지), 비트별 컷 전환 + 손 SVG 오버레이 + 스피드라인 + 행 단위 타이포 리빌, 전부 스크롤 연동. 톤 셰이딩+OutlineEffect(애니 질감). reduced-motion/WebGL 실패 시 정적 텍스트 폴백 내장. Three.js r168 + OutlineEffect는 `assets/vendor/` 번들(importmap, CDN 미의존). **`rally.html`(2D 측면)과 `look-test.html`(시안 재현)은 참고용으로 유지.** index.html 통합(메인 승격) 여부는 미정 — 사용자 결정 대기.
