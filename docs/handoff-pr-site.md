...
- **연출**: 배구 랠리(서브→리시브→토스→스파이크→꽂힘)를 스크롤 내러티브로, 공이 스크롤을 따라 궤적을 그림. 사용자 결정으로 **3D POV**가 본선: **`rally3d.html`** (Three.js 로우폴리 — 코트·네트·빈 캐릭터 4인·클래식 스월 공·밥그릇 엔딩 전부 프리미티브+캔버스 텍스처 코드 생성, 외부 에셋 없음. 카메라는 국면별 키프레임 보간 + lerp 스무딩). Three.js는 `assets/vendor/three.module.min.js`로 번들(CDN 미의존, r168). **`rally.html`은 2D 측면 버전** — 저사양/reduced-motion 폴백 후보로 유지. **2026-09-16 결정: `rally3d.html`을 `index.html`로 승격.** 냉정 평가·보완 레퍼런스·작업 순서는 `docs/rally-review.md`.
