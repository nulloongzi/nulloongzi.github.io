...
**P0 완료 (2026-09-16).** `rally3d.html` → `index.html` 승격 끝. 옛 `index.html` 내용은 후반부
"만드는 것들 · 연락" 섹션으로 흡수했다. `rally3d.html` 은 내용이 그대로 `index.html` 이 됐으므로 삭제.

| 항목 | 결과 |
|---|---|
| D 메타·OG·파비콘·타이틀 | 완료. `(프로토타입)` 제거, OG 제목/설명을 랠리 기준으로, `og:locale:alternate` 추가 |
| D 프로젝트·채널 | 완료. 누룽지도·안치기·**AI 심판(카드 되살림)** + 인스타·유튜브·링크트리 |
| D KO/EN | 완료. **인터뷰 전문 영문 번역.** 방식은 `data-ko/data-en` 속성이 아니라 `[data-l="ko"]`/`[data-l="en"]` 형제 블록 — 본문에 `<strong>`·따옴표가 섞여 속성값 이스케이프가 위험했다 |
| C 폰트 | 완료. Pretendard variable dynamic-subset CDN |
| E 폴백 | 완료. **계획 변경**: `rally.html` 로 보내지 않고 **제자리 degrade**. 리다이렉트하면 후반부 섹션(프로젝트·연락)을 잃는다. reduced-motion 이면 Three.js 모듈을 **아예 안 받고**(685KB 절약) `body.no3d` 그라데이션 배경. WebGL 실패도 같은 경로 |
| E 티어 | 완료. coarse pointer·deviceMemory≤4·cores≤4 → shadow 1024·PCF·DPR 1.5·antialias off |
| E 로딩 | 완료. 캔버스는 첫 프레임 뒤 페이드인, 히어로 텍스트는 즉시 |
| H AI 심판 | **카드 되살림.** 인터뷰의 "다음으로 준비하는 게 AI 심판" 과 일치시킴 |
| I 잔재 | 완료. 죽은 `Math.max(...)*0`, 하드코딩 `46vh`, 국면 제목 `div`→`h2`, 캔버스 `aria-hidden`. 홉 거리 계산도 매 프레임 → 1회로 |
| F 진행 표시 | **P1 이었으나 선반영.** 국면 5개 레일 + 현재 표시, 후반부 진입 시 자동 숨김 |

**브라우저 실측**(Chromium, 데스크톱 1280×800 / 모바일 390×844 DPR3 / reduced-motion): 페이지 오류 0,
3D·no3d 분기 정상, 레일 5개, 연락처 3개. 실측에서 잡아 고친 것 두 가지 —

- **모바일 히어로 제목이 네트·선수와 겹쳐 안 읽혔다.** 히어로 카드가 투명이라 3D 위에 맨몸으로 얹혀 있었다.
  → 부드러운 크림 스크림 + text-shadow.
- **세로 화면에서 코트가 잘렸다.** 세로에선 수평 화각이 좁아진다.
  → 가로 화각을 유지하도록 `fov` 를 종횡비에 맞춰 넓힘(상한 78°).
- 좁은 화면에서 레일이 카드와 겹쳐 오른쪽 여백 분리.

**P1 A·B 완료 (2026-09-16).**

- **A 랠리를 실제로 일으킴.** 핵심은 화질이 아니라 **타이밍**이었다. 선수들이 *극적인 포즈로 얼어* 있어서
  고장으로 읽혔던 것 — 정지 자체가 문제가 아니었다. 그래서 **기본을 '준비 자세'로 바꾸고, 공이 닿는
  순간 전후에만** 동작하게 했다. 서브(토스→백스윙→임팩트), 디그(낮춰 받고 일어남), 세트(두 손 밀어
  올림), 스파이크(도움닫기→점프→내려침). 전역 진행도 `g = (국면índex + t)/7` 위에 각 선수의 접점을
  두고 `s`(-1 준비 → 0 임팩트 → +1 복귀) 키프레임 트랙으로 보간. 외부 에셋 0 유지 — 리깅·mocap 없이
  팔 피벗 회전과 y 이동만으로 낸다. 정지 구간엔 미세한 숨쉬기.
- **B 프레이밍.** 카드를 넓은 화면에서 **왼쪽으로 몰고**, `camera.setViewOffset()` 으로 프러스텀을
  오른쪽(세로 화면은 위쪽)으로 밀어 **카드가 차지한 쪽을 비웠다.** 카메라 키프레임을 다시 그리지 않고
  구도를 얻는 방법. 추가로 세로 화면 달리(×1.42)와 스파이크 키프레임 후퇴 — 실측에서 피사체가 화면을
  꽉 채우고 얼굴이 잘렸다.

**남은 판단 — ~~`rally.html`(2D)~~ → 삭제 완료 (2026-09-16).** 아래 기록은 삭제 전 상태.: 폴백을 제자리 degrade 로 바꿔서 이제 어디서도 링크되지 않는다(고아).
인터뷰 원고 사본을 들고 있어 방치하면 본문과 어긋난다. 지울지, 저사양용으로 다시 물릴지 정할 것.
(`404.html` 이 고아로 잡히는 건 정상 — Pages 가 알아서 쓴다.)
